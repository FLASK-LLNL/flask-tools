...
def apply_exact_rules(results: list[ToolResult]) -> ReactionGrade:
    ...
    if mass is not None:
        ...
        if energy is None:
            print(
                f'Warning the rule for "mass.status is ToolStatus.POTENTIAL" is somewhat arbitrary'
            )
            ...
            return ReactionGrade(
                ...
                short_reason=f"exact.mass_potential-{missing_product_confidence or 'unknown'}",
                results=results,
                comment="Reaction depends on a possible omitted species to satisfy mass balance.",
            )
        else:
            ...
            if mass.status is ToolStatus.POTENTIAL and energy.status is ToolStatus.PASS:
                ...
                missing_product_confidence = mass.data.get("missing_product_confidence")
                final_grade = mapping.get(
                    missing_product_confidence, FinalGrade.UNCERTAIN
                )
                return ReactionGrade(
                    final_grade=final_grade,
