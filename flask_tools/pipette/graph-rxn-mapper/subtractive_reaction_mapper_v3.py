...
def auto_respect_atom_maps(
    reactants: Sequence[ReactantComponent], product_mol: Chem.Mol, cfg: MapperConfig
) -> bool:
...
def atom_compatible_attrs(
    a: Mapping[str, Any], b: Mapping[str, Any], cfg: MapperConfig, respect_maps: bool
) -> bool:
...
def bond_compatible_attrs(
    a: Mapping[str, Any], b: Mapping[str, Any], cfg: MapperConfig
) -> bool:
    if (
...
def atom_compatible_mol(
    ra: Chem.Atom, pa: Chem.Atom, cfg: MapperConfig, respect_maps: bool
) -> bool:
    return atom_compatible_attrs(atom_attrs(ra), atom_attrs(pa), cfg, respect_maps)


def bond_compatible_mol(rb: Chem.Bond, pb: Chem.Bond, cfg: MapperConfig) -> bool:
...
def copy_count_for_reactant(rc: ReactantComponent, cfg: MapperConfig) -> int:
...
def bond_environment_break_penalty(
    mol: Chem.Mol, begin_atom: int, end_atom: int, cfg: MapperConfig
) -> float:
